...
> AI Agent 指令文件。执行任务前优先查阅 `.claude/skills/` 下各 Skill 定义，获取完整交互流程与业务规则。
...
核心数据源：
- `data/eps3_spare.xlsx` — 备件输机价格（约 15.7 万行，40 列）；`通知书状态`区分"已审批"/"审批中"
- `data/eps3_mass.xlsx` — 量产件输机价格（约 17.3 万行，30 列）；`状态`区分"已发布"/"已确认"/"已审批"

## 检查能力总览

共 8 个 Notebook，分为三大类：
...
| 3 | 备件输机未联动检查 | `备件输机(应联动)_未联动检查.ipynb` | spare | `备件输机(应联动)未联动清单/` | 在线表格追加 |
...
| 8 | 备件与量产件价格差异分析 | `mass_spare_difference.ipynb` | 含 `input()`，须在 VSCode 中逐 cell 执行 |
...
├── .gitignore
├── .claude/
│   ├── .mcp.json                 # MCP 配置：ppt-mcp, powerbi-microsoft
│   └── skills/                   # 8 个 Skill 定义（优先于 notebook 实现）
├── data/                         # 只读输入数据（7 个 Excel）
├── notebooks/                    # 分析 Notebook（8 个 .ipynb）
├── output/                       # 输出结果（7 个业务子目录）
│   ├── 联动标识检查结果/
│   ├── 价格通知书_需商折/
│   ├── 备件输机(应联动)未联动清单/
│   ├── 备件高于同期量产件清单/
...
| 命令 | Skill 文件 | 交互流程 |
|------|-----------|---------|
| `/备件输机(应联动)未联动检查` | `备件输机(应联动)未联动检查.md` | 询问日期区间 → Python 执行 → 报告 → 询问飞书表格追加 |
| `/价格通知书_需商折` | `价格通知书_需商折.md` | 询问日期区间 → Python 执行 → 报告 → 询问飞书表格追加 |
| `/工程设变价格跟踪` | `工程设变价格跟踪.md` | 询问日期区间 → Python 执行 → 报告 → 询问飞书群消息 |
| `/专用工装到期剥离` | `专用工装到期剥离.md` | 全量扫描 → Python 执行 → 报告 → 询问飞书群消息 |
...
- `.claude/skills/*.md` 优先级高于 notebook 实现，执行时以 skill 定义为准
